...
abspath = '../../'
dpath = os.path.join(abspath, '../data/snuh/')
cpath = os.path.join(abspath, '../data/concepts')
spath = os.path.join(abspath, 'usedata/snuh/')
spath_m = os.path.join(abspath, 'usedata/mimic/')
tpath = os.path.join(abspath, 'usedata/snuh/tokens')
...
# 병렬 처리
if not os.path.exists(os.path.join(spath, 'alldiags.pkl')):
    import pandas as pd
    if not os.path.exists(os.path.join(dpath, 'ecgpath.csv')):
        all_files = []
        base_dir = '/home/coder/workspace/nas1_ECG_xml/ECG_xml_MAIN'
        for nf in tqdm(os.listdir(base_dir)):
            for file in os.listdir(os.path.join(base_dir, nf)):
                if '.xml' == file[-4:]:
                    all_files.append(os.path.join(base_dir, nf, file))
        person = pd.read_csv(os.path.join(dpath, 'person.csv'))
        all_files1 = [a for a in tqdm(all_files) if int(a.split('/')[-1].split('_')[0]) in person['person_id'].values]
        pd.DataFrame(all_files1, columns=['fpath']).to_csv(os.path.join(dpath, 'ecgpath.csv'), index=None)

    filepaths = pd.read_csv(os.path.join(dpath, 'ecgpath.csv'))['fpath'].values
    chunks = np.array_split(filepaths, 64)
    alldiags = ray.get([process_file.remote(chunk) for chunk in chunks])
    picklesave(alldiags, os.path.join(spath, 'alldiags.pkl'))
else:
    alldiags = pickleload(os.path.join(spath, 'alldiags.pkl'))
_alldiags = []
for a in tqdm(alldiags):
    for aa in a:
        for aaa in aa[1]:
            _alldiags.append([aa[0], aaa])
alldiags = mpd.DataFrame(_alldiags, columns=['path', 'report'])
alldiags['record_datetime'] = mpd.to_datetime(
    alldiags['path'].apply(lambda x: 
        x.split('/')[-1].split('_')[1] + x.split('/')[-1].split('_')[2].replace('.xml', '')),
    format='%Y%m%d%H%M'
)
alldiags['person_id'] = alldiags['path'].apply(lambda x: x.split('/')[-1].split('_')[0]).astype(int)
alldiags['report'] = [str(i) for i in alldiags['report']]


ecgidx = alldiags[['report']].drop_duplicates().set_index(['report'])
ecgidx['ecgidx'] = np.arange(len(ecgidx))# Merging EHR with ECG records and extracting outcomes
ecgidx['idx'] = [f'ecg_{i}' for i in range(len(ecgidx))]
ecgidx.to_csv('./ecgidx_snuh.csv')
...
alldiags['concept_id'] = ecgidx.loc[alldiags['report'].values]['idx'].values
alldiags['domain'] = 'ecg'
ecgs = alldiags[['person_id', 'domain', 'concept_id', 'record_datetime', 'path']]
ecgs.columns = list(allr.columns) + ['study_id']

allr = allr[allr['person_id'].isin(ecgs['person_id'])]
ecgs = ecgs[ecgs['person_id'].isin(allr['person_id'])]
allr['study_id'] = None
...
alldf.to_csv(os.path.join(spath, 'ecg_index_time_test.csv'), index=None)
...
allrecords.to_csv(os.path.join(spath, 'allrecords_ecg_test.csv'), index=None)
print('Done')
...
for name, _allr in allr_dict.items():
    ...
    savepath = os.path.join(tpath, 'Finetuning', f'CVD_snuh_{name}.pkl')
    ...
    vocab = torch.load(os.path.join(spath_m, 'vocab.pt'))
